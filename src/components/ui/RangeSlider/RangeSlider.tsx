...
export interface RangeSliderProps {
  ...
  onThumbDragStart?: (val: number[]) => void;
  onThumbDragEnd?: (val: number[]) => void;
  onRangeDragStart?: (val: number[]) => void;
  onRangeDragEnd?: (val: number[]) => void;
  disabled?: boolean;
...
class RangeSlider extends PureComponent<RangeSliderProps> {
  ...
  setValue(newValue, forceSet = false, callback = true) {
    ...
    const currentValue = this.setMinMaxProps(
    ...
    this.input[this.index.max].current.value =
      this.thumbDrag || forceSet
        ? newValue.max
        : newValue.min + this.rangeWidth;
    this.syncValues();
  ...
  updateThumbs() {
    this.iterateMinMaxProps((_) => {
      this.thumb[this.index[_]].current.style[
        ...
        (0.5 - (this.value[_] - this.options.min) / this.maxRangeWidth) *
        this.ifVerticalElse(this.thumbHeight, this.thumbWidth)[_]
      }px)`;
    });
  }
  ...
  updateRange() {
    ...
    const deltaOffset =
      ((0.5 - (this.value.min - this.options.min) / this.maxRangeWidth) *
        this.ifVerticalElse(this.thumbHeight, this.thumbWidth).min) /
      this.ifVerticalElse(
        elementBounds.bottom - elementBounds.top,
        elementBounds.right - elementBounds.left
      );
    const deltaDimension =
      ((0.5 - (this.value.max - this.options.min) / this.maxRangeWidth) *
        this.ifVerticalElse(this.thumbHeight, this.thumbWidth).max) /
      this.ifVerticalElse(
        elementBounds.bottom - elementBounds.top,
        elementBounds.right - elementBounds.left
      );
    this.range.current.style[this.ifVerticalElse("top", "left")] = `${
      ((this.value.min - this.options.min) / this.maxRangeWidth + deltaOffset) *
      100
    }%`;
  ...
  currentPosition(e, node) {
    ...
    const currPos =
      ((this.ifVerticalElse(
        nodeBounds.top - elementBounds.top,
        nodeBounds.left - elementBounds.left
      ) +
    ...
    if (setFocus) {
      ...
      if (this.options.thumbsDisabled[0]) {
        if (currPos >= this.value.min) {
          this.setValue(
            this.setMinMaxProps(this.value.min, currPos),
            true,
            !repeat
          );
        ...
        if (currPos <= this.value.max) {
          this.setValue(
            this.setMinMaxProps(currPos, this.value.max),
            true,
            !repeat
          );
          this.initiateThumbDrag(
        ...
        if (deltaMin === deltaMax) {
          ...
          this.setValue(
            this.setMinMaxProps(
              deltaMin < deltaMax ? currPos : this.value.min,
              deltaMax < deltaMin ? currPos : this.value.max
            ),
            true,
            !repeat
          );
          nearestThumbIndex =
            deltaMin < deltaMax ? this.index.min : this.index.max;
        }
        this.initiateThumbDrag(
          e,
          nearestThumbIndex,
          this.thumb[nearestThumbIndex].current,
          !repeat
        );
      }
      if (repeat) {
  ...
  render() {
    ...
    return (
      <div
        ...
        id={this.props.id}
        ref={this.element}
        className={clsx("range-slider", this.props.className)}
      >
        <input
          ref={this.input[0]}
          type="range"
...
const Wrapper = (props: RangeSliderProps) => {
